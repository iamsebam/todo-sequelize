  ...
  if (todoList) {
    todoList.addEventListener('click', e => {
      if (e.target.matches('li')) {
        ...
        req.open('DELETE', '/todos/todo/' + id, true)
        req.onload = () => {
        ...
        if (
          !li.classList.contains('li-active') &&
          e.target.classList.contains('success')
        ) {
        ...
        updateTodo(id, data)
      } else if (e.target.matches('.fa-save')) {
        // Save button
        id = e.target.parentElement.parentElement.dataset.todo_id
        ...
        let li = e.target.parentElement.parentElement
        let panel =
          '<span class="todo-panel"><i class="fas fa-check"></i><i class="fas fa-edit"></i><i class="fas fa-trash-alt"></i></span>'
...
function updateTodo(id, data) {
  const req = new XMLHttpRequest()
  req.open('PATCH', '/todos/todo/' + id, true)
